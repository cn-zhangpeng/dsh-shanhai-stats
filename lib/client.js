...
.cc-sidebar-action { display:flex; flex-direction:column; align-items:center; gap:2px; width:100%; padding:6px 0; border-radius:8px; cursor:pointer; font-size:11px; color:var(--dsw-alias-label-secondary); transition:background-color .15s ease, color .15s ease; }
.cc-sidebar-action:hover { background:var(--dsw-alias-bg-layer-2); color:var(--dsw-alias-label-primary); }
.cc-sidebar-action-icon { width:16px; height:16px; color:var(--dsw-alias-label-secondary); flex:none; }
.cc-sidebar-action:hover .cc-sidebar-action-icon { color:var(--dsw-alias-label-primary); }
.cc-sidebar-action-main { display:flex; flex-direction:column; gap:1px; min-width:0; }
.cc-sidebar-action-label { font-size:11px; }
.cc-sidebar-action-value { font-size:12px; font-weight:600; color:var(--dsw-alias-label-primary); }
...
`
    ...
    function UsagePage(props) {
      ...
      const [range, setRange] = React.useState({ kind: '7d' })
    ...
    function SidebarToday() {
      ...
      return React.createElement(React.Fragment, null,
        React.createElement('div', { className: 'cc-sidebar-action', onClick: () => setOpen(true), title: '查看今日用量' },
          ...
          React.createElement('div', { className: 'cc-sidebar-action-main' },
            ...
            React.createElement('span', { className: 'cc-sidebar-action-value', style: { whiteSpace: 'nowrap', textAlign: 'center' } }, todayStats === null ? '加载中…' : fmtCompact(total)),
          ),
        ),
    ...
    exports.apply = (ctx) => {
      ...
      slots.inject('sidebar.footer.action', () => slots.register(
        { name: 'sidebar.footer.action', id: 'shanhai-stats-today', order: 1 },
        () => React.createElement(SidebarToday),
      ))
    }
