...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function rangeAgg(stats, range) {
      const empty = { totals: { turns: 0, msgs: 0, sessions: 0, input: 0, output: 0, cacheRead: 0, cacheWrite: 0, reasoning: 0 }, perWs: [], perModel: [] }
      ...
      const t = { turns: 0, msgs: 0, sessions: 0, input: 0, output: 0, cacheRead: 0, cacheWrite: 0, reasoning: 0 }
      const perWs = new Map()
      ...
      for (const day of days) {
        ...
        for (const w of (day.byWorkspace || [])) {
          ...
          if (p === undefined) { p = { workspaceId: w.workspaceId, turns: 0, msgs: 0, input: 0, output: 0, cacheRead: 0, cacheWrite: 0, reasoning: 0 }; perWs.set(w.workspaceId, p) }
          p.input += w.input || 0
          p.output += w.output || 0
...
.cc-table-head { display:flex; align-items:center; justify-content:space-between; gap:12px; margin-bottom:10px; }
.cc-tabs { display:flex; gap:4px; background:var(--dsw-alias-bg-layer-2); border-radius:6px; padding:2px; }
.cc-tab { border:none; background:transparent; color:var(--dsw-alias-label-secondary); font-size:12px; padding:4px 10px; border-radius:4px; cursor:pointer; }
.cc-tab.cc-active { background:var(--dsw-alias-bg-layer-1); color:var(--dsw-alias-label-primary); }
...
.cc-line-wrap { position:relative; width:100%; height:180px; padding:10px 24px 0 0; box-sizing:border-box; }
.cc-line { width:100%; height:100%; overflow:visible; }
.cc-axis-y { position:absolute; left:0; top:10px; bottom:24px; width:36px; display:flex; flex-direction:column; justify-content:space-between; align-items:flex-end; padding-right:6px; font-size:10px; color:var(--dsw-alias-label-tertiary); pointer-events:none; }
.cc-axis-y span { line-height:1; }
.cc-line-area { position:absolute; left:36px; right:24px; top:10px; bottom:24px; }
.cc-line-xlabels { position:absolute; left:36px; right:24px; bottom:0; height:20px; }
...
.cc-line-dots { position:absolute; inset:0; pointer-events:none; }
.cc-line-dot { position:absolute; width:6px; height:6px; border-radius:50%; transform:translate(-50%, -50%); background:var(--dsw-alias-bg-layer-1); border:2px solid; cursor:pointer; pointer-events:auto; transition:width .15s ease, height .15s ease; }
.cc-line-dot:hover { width:10px; height:10px; }
.cc-line-dot.cc-line-active { width:10px !important; height:10px !important; }
...
`
    ...
    function UsagePage(props) {
      ...
      const animatedMsgs = useCountUp(agg.totals.msgs)
      ...
      const activeKeys = BAR_KEYS.filter((k) => chartDays.some((cd) => cd.tokens[k] > 0))
      let maxToken = 1
      for (const cd of chartDays) {
        for (const k of activeKeys) if (cd.tokens[k] > maxToken) maxToken = cd.tokens[k]
      }
      const chartMax = Math.max(4, Math.ceil(maxToken * 1.2))
      const LINE_W = 560
      const LINE_H = 130
      const LINE_PAD = 8
      const linePoints = {}
      ...
      chartDays.forEach((cd, i) => {
        const x = chartDays.length === 1 ? 50 : (LINE_PAD + (i / (chartDays.length - 1)) * (LINE_W - LINE_PAD * 2)) / LINE_W * 100
        const active = lineHover !== null && lineHover.date === cd.date
        for (const k of activeKeys) {
          const y = LINE_H - LINE_PAD - (cd.tokens[k] / chartMax) * (LINE_H - LINE_PAD * 2)
          const yPct = y / LINE_H * 100
          lineDots.push(React.createElement('div', {
            key: k + '-' + i,
            className: 'cc-line-dot' + (active ? ' cc-line-active' : ''),
            style: { left: x + '%', top: yPct + '%', background: BAR_COLORS[k], borderColor: BAR_COLORS[k] },
      ...
      const lineLegend = activeKeys.map((k) => React.createElement('span', { key: k, className: 'cc-legend-item' },
      ...
      const lineSvg = React.createElement('svg', { className: 'cc-line', viewBox: '0 0 ' + LINE_W + ' ' + LINE_H, preserveAspectRatio: 'none' },
        ...activeKeys.map((k) => React.createElement('polyline', { key: k, className: 'cc-line-' + k, points: linePoints[k], fill: 'none', stroke: BAR_COLORS[k], strokeWidth: 2, strokeLinejoin: 'round', strokeLinecap: 'round' })),
      )
      const lineDotsLayer = React.createElement('div', { className: 'cc-line-dots' }, ...lineDots)
      const lineXLabels = chartDays.map((cd, i) => {
        ...
        return React.createElement('span', { key: cd.date, className: 'cc-line-xlabel' + (show ? '' : ' cc-hide'), style: { left: (chartDays.length === 1 ? 50 : (i / (chartDays.length - 1)) * 100) + '%' } }, cd.mday)
      })
      ...
      const yAxis = React.createElement('div', { className: 'cc-axis-y' },
        yTicks.map((t) => React.createElement('span', { key: t.value, style: { position: 'absolute', top: t.top, transform: 'translateY(-50%)' } }, fmtAxisY(t.value))),
      )
      ...
      const cellElements = hmDays.map((cell, i) => {
        ...
        const count = day !== undefined ? day.msgs || 0 : 0
      ...
      for (const m of models) {
        ...
        if (g === undefined) {
          g = { model: key, providers: [], input: 0, output: 0, cacheRead: 0, cacheWrite: 0, reasoning: 0, msgs: 0 }
          modelMap.set(key, g)
          modelGroups.push(g)
        }
      ...
      modelGroups.sort((a, b) => modelTotal(b) - modelTotal(a))
      ...
      if (lineHover !== null && lineHover !== undefined) {
        ...
        tip = React.createElement('div', {
          ...
          activeKeys.map((k) => React.createElement('div', { key: k, className: 'cc-tip-row' },
        ...
        if (day !== undefined) {
          ...
          const sorted = pw.slice().sort((a, b) => (b.msgs || 0) - (a.msgs || 0))
          rowsContent = sorted.map((entry) => {
            ...
            return React.createElement('div', {
              ...
              React.createElement('span', { className: 'cc-n' }, (entry.msgs || 0) + ' 次'),
            )
          })
      ...
      return React.createElement('div', { className: 'cc-page' },
        ...
        isEmpty ? React.createElement('div', { className: 'cc-panel' },
          ...
          React.createElement('div', { className: 'cc-cards' },
            ...
            card('总请求次数', fmtCount(animatedMsgs), range.kind === 'all' ? '与模型的请求数累计' : rangeLabel + '内的请求数', 2.2),
            ...
            chartDays.length === 0
              ...
              : React.createElement('div', { className: 'cc-chart-scroll' },
                React.createElement('div', { className: 'cc-line-wrap' },
                  yAxis,
                  React.createElement('div', { className: 'cc-line-area' }, lineSvg, lineDotsLayer),
          ...
          React.createElement('div', { className: 'cc-panel' },
            React.createElement('div', { className: 'cc-hm-head' },
              React.createElement('div', { className: 'cc-legend' },
                ['0', '1–50', '50–200', '200–500', '500+'].map((text, l) => React.createElement(React.Fragment, { key: l },
            ...
            React.createElement('div', { className: 'cc-note', style: { marginTop: 10 } }, '口径：每完成一次与模型的请求点亮一次（含子代理会话）；悬停查看按工作区明细。'),
          ),
          React.createElement('div', { className: 'cc-panel' },
            React.createElement('div', { className: 'cc-table-head' },
              React.createElement('h3', { className: 'cc-panel-title', style: { margin: 0 } }, '用量明细（' + rangeLabel + '）'),
              React.createElement('div', { className: 'cc-tabs' },
                React.createElement('button', { className: 'cc-tab' + (groupBy === 'provider' ? ' cc-active' : ''), onClick: () => setGroupBy('provider') }, '按供应商'),
                React.createElement('button', { className: 'cc-tab' + (groupBy === 'model' ? ' cc-active' : ''), onClick: () => setGroupBy('model') }, '按模型'),
              ),
            ),
            groupBy === 'provider'
              ? (providers.length === 0
              ...
              : (modelGroups.length === 0
                ? React.createElement('div', { className: 'cc-empty' }, '暂无模型用量数据')
                : React.createElement(React.Fragment, null,
                  React.createElement('div', { className: 'cc-hrow' },
                    React.createElement('div', {}, '模型'),
                    React.createElement('div', { className: 'cc-num' }, '供应商'),
                    React.createElement('div', { className: 'cc-num' }, '请求数'),
                    React.createElement('div', { className: 'cc-num' }, 'Tokens'),
                    React.createElement('div', { className: 'cc-num' }, '缓存命中'),
                  ),
                  modelGroups.map((g) => {
                    const gTotal = modelTotal(g)
                    return React.createElement('div', { key: g.model, className: 'cc-row' },
                      React.createElement('div', { className: 'cc-grp-name' },
                        React.createElement('span', { className: 'cc-dot', style: { background: providerColor(g.providers[0] || '') } }),
                        g.model,
                      ),
                      React.createElement('div', { className: 'cc-num' }, g.providers.length),
                      React.createElement('div', { className: 'cc-num' }, fmtCompact(g.msgs)),
                      React.createElement('div', { className: 'cc-num' }, fmtCompact(gTotal)),
                      React.createElement('div', { className: 'cc-num' }, fmtCompact(g.cacheRead)),
                    )
